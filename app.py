...
if st.button("Generuj Dokumenty", type="primary"):
    ...
    if len(nip) != 10:
        ...
        if not dane_z_api:
            ...
            def podziel_glowny_dokument(pdf_bytes, czy_krs):
                """
                Dzieli spłaszczony PDF głównego dokumentu (KRS/JDG) na dwa osobne pliki:
                - ORK: zawsze pierwsze 2 strony
                - Oświadczenie: strona 3 (Spółka/KRS) albo strony 3-4 (JDG)
                Nie dotyka Pełnomocnictwa ani Załącznika.
                """
                if not pdf_bytes:
                    return None, None
                doc_flat = fitz.open(stream=pdf_bytes, filetype="pdf")
            ...
            bufor_glowny_pelny = generuj_plik(plik_glownego)
            st.session_state.bufor_ork, st.session_state.bufor_glowny = podziel_glowny_dokument(bufor_glowny_pelny, czy_krs)
